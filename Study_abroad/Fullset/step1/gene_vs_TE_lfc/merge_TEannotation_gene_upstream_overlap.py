...
for i in chr_list:
 ...
 for j in range(len(TE_sub_data)):
  ...
  for k in range(len(Gene_sub_data)):
   ...
   if(TE_column.TE_start<Gene_column.Transcript_start and \
   Gene_column.Transcript_start<TE_column.TE_end and \
...
TE_transcript_data.to_csv("/mnt/Results/upstream_transcripts_TE_overlap.txt",sep="\t",index=False)
