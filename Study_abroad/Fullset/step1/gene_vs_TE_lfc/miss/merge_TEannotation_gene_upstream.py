...
for i in chr_list:
 ...
 for j in range(len(TE_sub_data)):
  ...
  for k in range(len(Gene_sub_data)):
   ...
   if(TE_column.TE_end<Gene_column.Transcript_start and \
   0<(Gene_column.Transcript_start-TE_column.TE_start) and \
   (Gene_column.Transcript_start-TE_column.TE_start)<threshold and \
   TE_column.TE_strand==Gene_column.Transcript_strand):
    ...
    for l in TE_column[1:]:
     append_data.append(l)
    append_data.append(Gene_column.Transcript_start-TE_column.TE_end)
...
TE_transcript_data.to_csv("/mnt/Results/upstream_transcripts_TE.txt",sep="\t",index=False)
