...
#   print(TE_chri_strand.iloc[start_pos+1])
   for j in range(start_pos,len(TE_chri_strand)):
    ...
    if(TE_col.TE_start>transcript_col.Transcript_end):#shakutori
     start_pos=j
     continue
    elif(transcript_col.Transcript_end>TE_col.TE_end):#finish if TE end site exceed Transcript end site
     break
    ##overlapped
    if(transcript_col.Transcript_end<TE_col.TE_end):
     input_line=[]
...
want_data_pd.to_csv("/mnt/Results/merge_transcript_TE_downstream_overlap.txt",sep='\t',index=False)
