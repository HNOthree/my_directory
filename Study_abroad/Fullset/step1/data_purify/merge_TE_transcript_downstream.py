...
for chr_i in chr_list:
 ...
 for strand in strands:
  ...
  transcript_chri_strand=transcript_chri_strand.sort_values("Transcript_start", ascending=False)
  TE_chri_strand=TE_chri_strand.sort_values("TE_start", ascending=False)
...
#   print(TE_chri_strand.iloc[start_pos+1])
   for j in range(start_pos,len(TE_chri_strand)):
    ...
    if(int(TE_col.TE_start-transcript_col.Transcript_end)>threshold):#shakutori
     start_pos=j
     continue
    elif(transcript_col.Transcript_end>TE_col.TE_end):#finish if TE start site exceed Transcript start site
     break
    ##not overlapped
    if(transcript_col.Transcript_end<TE_col.TE_start):
     input_line=[]
     ...
     for k in TE_col[0:5]:
      input_line.append(k)
     ##distance
     input_line.append(TE_col.TE_start-transcript_col.Transcript_end)
...
want_data_pd.to_csv("/mnt/Results/merge_transcript_TE_downstream.txt",sep='\t',index=False)
