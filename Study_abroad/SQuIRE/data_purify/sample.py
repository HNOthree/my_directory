 ...
 for i in range(len(TE_candidates)):
   ...
   for j in range(len(gene_candidates)):
    ...
    if(TE.Start_TE>a_gene.Gene_end):
     ...
     gene_exp_col=''
     for k in range(0,len(gene_exp)):
      a_candidate=gene_exp.iloc[k]
      if(a_candidate.Gene==a_gene.Gene_id):
       print(a_candidate)
       print(a_gene.Gene_id)
       gene_exp_col=a_candidate
       break
