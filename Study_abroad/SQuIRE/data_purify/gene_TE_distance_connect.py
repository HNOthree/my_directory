import pandas as pd
import math
TE_info=pd.read_csv("DESeq2_TE_separated.txt",sep='\t')
gene_info=pd.read_csv("Gene.txt",sep='\t')
gene_exp=pd.read_csv("Latipes_Expression_table.csv",sep='\t')
...
for chromosome in chr_list:
 ...
 for i in range(len(TE_candidates)):
   ...
   if(len(upstream_list)>0):
    ...
    upstream_list=upstream_list[upstream_list.Distance<500000]
   ...
   if(len(downstream_list)>0):
    ...
    #downstream_list=downstream_list.sort_values(by=["Distance"],sort=True)
    #downstream_list=downstream_list.reset_index()
    #downstream_list=downstream_list.drop("index",axis=1)
    downstream_list=downstream_list[downstream_list.Distance<500000]
    wantdata=wantdata.append(downstream_list)
wantdata=wantdata[wantdata.Family!="Ignore"]
wantdata.to_csv("Gene_TE_distance.txt",index=False,sep='\t')
