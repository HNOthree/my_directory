...
myfunc<-function(load_directory,save_directory){
  name<-paste(directory,"/correlation_coefficiency_between_100k.txt",sep='')
  ...
  Label=paste(Label,"\n(n=")
  Label=paste(Label,as.character(nrow(all_set[all_set$Region=="ALL",])))
  Label=paste(Label,as.character(nrow(all_set[all_set$Region=="female",])))
  Label=paste(Label,as.character(nrow(all_set[all_set$Region=="male",])))
  Label=paste(Label,as.character(nrow(all_set[all_set$Region=="non_biased_region",])))
  Label=paste(Label,")")
  all_set<-transform(all_set, distance=Label )
  for(i in c(2,3,4,5,6,7)){#length
    length=length_seq[i]
    subset=table[table$Distance==length ,]
    Label=paste(as.character(length_seq[i-1]+1),"~")
    Label=paste(Label,as.character(length_seq[i]))
    Label=paste(Label,"\n(n=")
    Label=paste(Label,as.character(nrow(subset[subset$Region=="ALL",])))
    Label=paste(Label,as.character(nrow(subset[subset$Region=="female",])))
    Label=paste(Label,as.character(nrow(subset[subset$Region=="male",])))
    Label=paste(Label,as.character(nrow(subset[subset$Region=="non_biased_region",])))
    Label=paste(Label,")")
  ...
  g <- g+theme(axis.text.x=element_text(size=rel(1.3), angle=90))
  ...
  name<-paste(directory,"/correlation_coefficiency_lessthan_100k.txt",sep='')
  Table = read.table(name, sep="\t", header=T,encoding="utf-8")
  table<-subset(Table,complete.cases(Table))
  #table<-table[table$Region!="ALL",]
  ...
  g <- ggplot(all_set, aes(x = distance, y = Correlation_coefficiency,colour=Region))
  g <- g + geom_boxplot(na.rm = TRUE)+theme_bw()+ylim(-1.0,1.0)+theme(axis.text.x=element_text(size=rel(1.3), angle=90))
  ...
  g <- ggplot(suball, aes(x = distance, y = Correlation_coefficiency))
  g <- g + geom_boxplot(na.rm = TRUE)+theme_bw()+ylim(-1.0,1.0)
  g <- g+theme(axis.text.x=element_text(size=rel(1.2), angle=90))
